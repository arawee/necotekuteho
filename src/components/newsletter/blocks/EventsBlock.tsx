...
export const EventsBlock = ({ block, onUpdate }: EventsBlockProps) => {
        ...
        <div className="space-y-8">
          {block.content.events?.map((event, index) => (
            <div key={index} className="relative pb-6 border-b last:border-b-0 group">
              ...
              <div className="absolute right-0" style={{ bottom: '16px' }}>
                <Dialog>
                  <DialogTrigger asChild>
                    <button 
                      className="text-black px-4 py-2 text-sm font-medium rounded-full cursor-pointer"
                      style={{ backgroundColor: '#FBFB62' }}
                    >
                      {event.buttonText}
                    </button>
                  </DialogTrigger>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const newEvents = [...(block.content.events || [])];
                            newEvents[index] = { ...newEvents[index], buttonText: e.target.value };
                            onUpdate({ ...block.content, events: newEvents });
                          }}
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const newEvents = [...(block.content.events || [])];
                            newEvents[index] = { ...newEvents[index], buttonUrl: e.target.value };
                            onUpdate({ ...block.content, events: newEvents });
                          }}
                        />
                      </div>
